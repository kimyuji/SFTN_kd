...
# Feature Transform Layer for SFTN
class FeatTrans(nn.Module):
    """Transform Layer for SFTN"""
    def __init__(self, s_shape, t_shape, use_relu=True):
        ...
        if s_H < t_H: 
            self.conv = nn.Conv2d(t_C, s_C, kernel_size=3, stride=2, padding=1)
        # to increase the size of the feature map
        elif s_H > t_H: 
            self.conv = nn.ConvTranspose2d(t_C, s_C, kernel_size=4, stride=2, padding=1)
        # to keep the same size of the feature map 
        elif s_H == t_H: 
            self.conv = nn.Conv2d(t_C, s_C, kernel_size=1)
        else:
            raise NotImplemented('student size {}, teacher size {}'.format(s_H, t_H))
        self.bn = nn.BatchNorm2d(t_C)
        self.relu = nn.ReLU(inplace=True)
